...
;;;
;;; lexers CSV/TSV Round-Trip Corpus Test
;;;
;;
;; Round-trip fidelity test for CSV and TSV files copied from the local
;; `/tmp/peek-delimited-corpus-large` snapshot.
...
(define corpus-root
  (or (getenv "PEEK_DELIMITED_CORPUS")
      "/tmp/peek-delimited-corpus-large"))
...
;; delimited-path? : path-string? -> boolean?
;;   Recognize CSV/TSV corpus files.
(define (delimited-path? path)
  (define text
    (path->string path))
  (regexp-match? #px"(?i:\\.(?:csv|tsv))$" text))

;; collect-corpus-files : path? -> (listof path?)
;;   Collect CSV/TSV corpus files from one source tree.
(define (collect-corpus-files root)
  (sort (for/list ([path (in-directory root)]
                   #:when (file-exists? path)
                   #:when (delimited-path? path))
          path)
        string<?
        #:key path->string))
...
;; run-lexers-delimited-roundtrip-tests : -> void?
;;   Execute the CSV/TSV corpus test or skip clearly if unavailable.
(define (run-lexers-delimited-roundtrip-tests)
  (cond
    [(not (directory-exists? corpus-root))
     (displayln
      (format "Skipping CSV/TSV round-trip test; corpus not found at ~a"
              corpus-root))]
    [else
     (define corpus-files
       (collect-corpus-files corpus-root))
     (check-true (pair? corpus-files)
                 (format "No CSV/TSV files found under ~a"
                         corpus-root))
     (for ([path (in-list corpus-files)])
       (match (roundtrip-result path)
         ...
         [(list 'timeout timed-out-path)
          (error 'lexers-delimited-roundtrip
...
(module+ test
  (run-lexers-delimited-roundtrip-tests))
