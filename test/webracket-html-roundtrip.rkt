...
;;;
;;; webracket HTML Round-Trip Corpus Test
;;;
;;
;; Round-trip fidelity test for HTML files copied from the local `../webracket`
;; checkout into `/tmp`.

;; run-webracket-html-roundtrip-tests : -> void?
;;   Copy the local `webracket` HTML corpus to `/tmp` and verify that
...
(define temp-root
  (build-path "/tmp" "peek-webracket-html-roundtrip"))
...
;;   Recognize HTML corpus files.
(define (corpus-path? path)
  (define text
    (path->string path))
  (regexp-match? #px"(?i:\\.(?:html?|htm))$" text))

;; collect-corpus-files : path? -> (listof path?)
;;   Collect HTML corpus files from one source tree.
...
;;   Copy all HTML corpus files to `/tmp` and return copied paths.
...
(define (assert-roundtrip-file path)
  ...
  (define rendered
    (with-handlers ([exn:fail?
                     (lambda (e)
                       (error 'webracket-html-roundtrip
                              "~a\n~a"
                              (path->string path)
                              (exn-message e)))])
