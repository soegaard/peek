...
;;;
;;; lexers Java Round-Trip Corpus Test
;;;
;;
;; Round-trip fidelity test for Java files copied from the local
;; `/tmp/lexers-java-corpus` snapshot.
...
;; java-path? : path-string? -> boolean?
;;   Recognize Java corpus files.
(define (java-path? path)
  (define text
    (path->string path))
  (regexp-match? #px"(?i:\\.java)$" text))

;; collect-corpus-files : path? -> (listof path?)
;;   Collect Java corpus files from one source tree.
...
;; run-lexers-java-roundtrip-tests : -> void?
;;   Execute the Java corpus test or skip clearly if unavailable.
(define (run-lexers-java-roundtrip-tests)
  (cond
    [(not (directory-exists? corpus-root))
     (displayln
      (format "Skipping Java round-trip test; corpus not found at ~a"
              corpus-root))]
    [else
     (define corpus-files
       (collect-corpus-files corpus-root))
     (check-true (pair? corpus-files)
                 (format "No Java files found under ~a"
                         corpus-root))
...
(module+ test
  (run-lexers-java-roundtrip-tests))
