...
;;;
;;; lexers TeX Round-Trip Corpus Test
;;;
;;
;; Round-trip fidelity test for TeX and LaTeX files copied from the local
;; `/tmp/lexers-tex-corpus` snapshot.
...
;; tex-path? : path-string? -> boolean?
;;   Recognize TeX and LaTeX corpus files.
(define (tex-path? path)
  (define text
    (path->string path))
  (regexp-match? #px"(?i:\\.(?:tex|cls|sty))$" text))

;; collect-corpus-files : path? -> (listof path?)
;;   Collect TeX corpus files from one source tree.
...
;; run-lexers-tex-roundtrip-tests : -> void?
;;   Execute the TeX corpus test or skip clearly if unavailable.
(define (run-lexers-tex-roundtrip-tests)
  (cond
    [(not (directory-exists? corpus-root))
     (displayln
      (format "Skipping TeX round-trip test; corpus not found at ~a"
              corpus-root))]
    [else
     (define corpus-files
       (collect-corpus-files corpus-root))
     (check-true (pair? corpus-files)
                 (format "No TeX files found under ~a"
                         corpus-root))
...
(module+ test
  (run-lexers-tex-roundtrip-tests))
