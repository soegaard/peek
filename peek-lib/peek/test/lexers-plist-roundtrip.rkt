...
;;;
;;; lexers Plist Round-Trip Corpus Test
;;;
;;
;; Round-trip fidelity test for plist files copied from the local
;; `/tmp/lexers-plist-corpus` snapshot.
...
;; corpus-file? : path-string? -> boolean?
;;   Recognize plist corpus files.
(define (corpus-file? path)
  (define text
    (path->string path))
  (not (regexp-match? #px"(?i:summary\\.txt)$" text)))

;; collect-corpus-files : path? -> (listof path?)
;;   Collect plist corpus files from one source tree.
(define (collect-corpus-files root)
  (sort (for/list ([path (in-directory root)]
                   #:when (file-exists? path)
                   #:when (corpus-file? path))
          path)
        string<?
        #:key path->string))
...
;; run-lexers-plist-roundtrip-tests : -> void?
;;   Execute the plist corpus test or skip clearly if unavailable.
(define (run-lexers-plist-roundtrip-tests)
  (cond
    [(not (directory-exists? corpus-root))
     (displayln
      (format "Skipping plist round-trip test; corpus not found at ~a"
              corpus-root))]
    [else
     (define corpus-files
       (collect-corpus-files corpus-root))
     (check-true (pair? corpus-files)
                 (format "No plist files found under ~a"
                         corpus-root))
...
(module+ test
  (run-lexers-plist-roundtrip-tests))
