...
;;;
;;
;; Round-trip fidelity test for C++ files copied from a local `/tmp`
;; corpus snapshot.
...
(define corpus-root
  (or (getenv "PEEK_CPP_CORPUS")
      "/tmp/peek-cpp-corpus-large"))
...
;; cpp-path? : path-string? -> boolean?
;;   Recognize C++ corpus files.
(define (cpp-path? path)
  (define text
    (path->string path))
  (regexp-match? #px"(?i:\\.(?:cpp|cc|cxx|cp|c\\+\\+|cppm|ixx|hpp|hh|hxx|h\\+\\+|ipp|tpp))$" text))
...
;; run-lexers-cpp-roundtrip-tests : -> void?
;;   Execute the C++ corpus test or skip clearly if unavailable.
(define (run-lexers-cpp-roundtrip-tests)
  (cond
...
(module+ test
  (run-lexers-cpp-roundtrip-tests))
