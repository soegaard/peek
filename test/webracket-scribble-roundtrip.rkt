...
;;;
;;; webracket Scribble Round-Trip Corpus Test
;;;
;;
;; Round-trip fidelity test for Scribble files copied from the local
;; `../webracket` checkout into `/tmp`.

;; run-webracket-scribble-roundtrip-tests : -> void?
;;   Copy the local `webracket` Scribble corpus to `/tmp` and verify that
...
(define temp-root
  (build-path "/tmp" "peek-webracket-scribble-roundtrip"))

;; Maximum number of Scribble files to check.
...
;;   Recognize Scribble corpus files.
(define (corpus-path? path)
  (define text
    (path->string path))
  (regexp-match? #px"(?i:\\.scrbl)$" text))

;; collect-corpus-files : path? -> (listof path?)
;;   Collect Scribble corpus files from one source tree.
...
;;   Keep the largest Scribble files when a limit is provided.
...
;;   Copy all Scribble corpus files to `/tmp` and return copied paths.
     ...
     (for ([path (in-list copied-paths)])
       (match (roundtrip-result path)
         ...
         [(list 'timeout timed-out-path)
          (error 'webracket-scribble-roundtrip
...
(module+ test
  (run-webracket-scribble-roundtrip-tests))
