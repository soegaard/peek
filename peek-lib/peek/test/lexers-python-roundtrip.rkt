...
;;;
;;; lexers Python Round-Trip Corpus Test
;;;
;;
;; Round-trip fidelity test for Python files copied from the local
;; `/tmp/lexers-python-corpus` snapshot.
...
;; python-path? : path-string? -> boolean?
;;   Recognize Python corpus files.
(define (python-path? path)
  (define text
    (path->string path))
  (regexp-match? #px"(?i:\\.(?:py|pyi|pyw))$" text))

;; collect-corpus-files : path? -> (listof path?)
;;   Collect Python corpus files from one source tree.
(define (collect-corpus-files root)
  (sort (for/list ([path (in-directory root)]
                   #:when (file-exists? path)
                   #:when (python-path? path))
          path)
        string<?
        #:key path->string))
...
;; run-lexers-python-roundtrip-tests : -> void?
;;   Execute the Python corpus test or skip clearly if unavailable.
(define (run-lexers-python-roundtrip-tests)
  (cond
    [(not (directory-exists? corpus-root))
     (displayln
      (format "Skipping Python round-trip test; corpus not found at ~a"
              corpus-root))]
    [else
     (define corpus-files
       (collect-corpus-files corpus-root))
     (check-true (pair? corpus-files)
                 (format "No Python files found under ~a"
                         corpus-root))
...
(module+ test
  (run-lexers-python-roundtrip-tests))
